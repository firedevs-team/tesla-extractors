...
/**
 * Extractor para el total de ventas retail y exportaciones
 * de Tesla en China
 */
class Extractor extends CNEVExtractor {
  constructor() {
    super({
      ...
      source: 'cnev_tesla_retail_and_export',
      fileext: 'json',
    });
  }
  ...
  async downloadFromPage(
    ...
    const articleUrl = await page.evaluate((month) => {
      ...
      for (const anchor of anchors) {
        ...
        if (text.startsWith(`TESLA CHINA CAR SALES IN ${MONTH_MAP[month]}:`)) {
    ...
    // Las exportaciones del mes en china
